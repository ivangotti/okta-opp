...
const validScimAttr = [ // array containing scim attributes supported by our plugin code. Empty array - all attrbutes are supported by endpoint
  ...
  //'phoneNumbers.mobile.type',
  //'user:custom.numbernew', //new attr added by TB
  "emails",         // accepts all multivalues for this key
  'emails.primary',  //added email TB
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      const insert = {
        ...
        //MobilePhone: (userObj.user:custom.numbernew) ? `'${userObj.user:custom.numbernew}'` : null,  ///mapped new attr to mobilenumber
        //MobilePhone: (userObj.phoneNumbers.work.value) ? `'${userObj.phoneNumbers.work.value}'` : null,
        MobilePhone: (userObj.phoneNumbers.mobile.value) ? `'${userObj.phoneNumbers.mobile.value}'` : null, ///added new phone number TB
        //Email: (userObj.emails.work.value) ? `'${userObj.emails.work.value}'` : null
        Email: (userObj.emails.primary.value) ? `'${userObj.emails.primary.value}'` : null //aadded okta emails
      }
...
scimgateway.modifyUser = async (baseEntity, id, attrObj, ctx) => {
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      if (attrObj.phoneNumbers.mobile.value !== undefined) {
        if (attrObj.phoneNumbers.mobile.value === '') sql += 'MobilePhone=null,'
        else sql += `MobilePhone='${attrObj.phoneNumbers.mobile.value}',`
      }
      if (attrObj.emails.primary.value !== undefined) {
        if (attrObj.emails.primary.value === '') sql += 'Email=null,'
        else sql += `Email='${attrObj.emails.primary.value}',`
      }
